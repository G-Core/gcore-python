...
def main() -> None:
    ...
    wait_for_storage_provisioning(client=gcore, storage_id=s3_storage_id)
    get_s3_storage(client=gcore, storage_id=s3_storage_id)
    new_access_key = create_s3_access_key(client=gcore, storage_id=s3_storage_id)
    list_s3_access_keys(client=gcore, storage_id=s3_storage_id)
    delete_s3_access_key(client=gcore, storage_id=s3_storage_id, access_key=new_access_key)
    list_s3_access_keys(client=gcore, storage_id=s3_storage_id)
    delete_s3_storage(client=gcore, storage_id=s3_storage_id)

    # SFTP storage operations
    sftp_storage_id = create_sftp_storage(client=gcore)
    wait_for_sftp_storage_provisioning(client=gcore, storage_id=sftp_storage_id)
    get_sftp_storage(client=gcore, storage_id=sftp_storage_id)
    regenerate_sftp_password(client=gcore, sftp_storage_id=sftp_storage_id)
    remove_sftp_password(client=gcore, sftp_storage_id=sftp_storage_id)
    reenable_sftp_password(client=gcore, sftp_storage_id=sftp_storage_id)
    delete_sftp_storage(client=gcore, storage_id=sftp_storage_id)
...
def wait_for_sftp_storage_provisioning(
    *, client: Gcore, storage_id: int, max_wait: int = 30, wait_interval: int = 2
) -> None:
...
def get_s3_storage(*, client: Gcore, storage_id: int) -> None:
...
def get_sftp_storage(*, client: Gcore, storage_id: int) -> None:
...
def create_s3_access_key(*, client: Gcore, storage_id: int) -> str:
    print("\n=== CREATE S3 ACCESS KEY ===")
    key = client.storage.object_storages.access_keys.create(storage_id=storage_id)
...
def list_s3_access_keys(*, client: Gcore, storage_id: int) -> None:
    print("\n=== LIST S3 ACCESS KEYS ===")
    for count, key in enumerate(client.storage.object_storages.access_keys.list(storage_id=storage_id), 1):
        print(f"  {count}. Access Key: {key.access_key}, Created: {key.created_at}")
    print("===========================")


def delete_s3_access_key(*, client: Gcore, storage_id: int, access_key: str) -> None:
    print("\n=== DELETE S3 ACCESS KEY ===")
    client.storage.object_storages.access_keys.delete(access_key, storage_id=storage_id)
    print(f"Deleted access key: {access_key}")
    print("============================")


def regenerate_sftp_password(*, client: Gcore, sftp_storage_id: int) -> None:
...
def remove_sftp_password(*, client: Gcore, sftp_storage_id: int) -> None:
...
def reenable_sftp_password(*, client: Gcore, sftp_storage_id: int) -> None:
...
def delete_s3_storage(*, client: Gcore, storage_id: int) -> None:
    print("\n=== DELETE S3 STORAGE ===")
...
def delete_sftp_storage(*, client: Gcore, storage_id: int) -> None:
    print("\n=== DELETE SFTP STORAGE ===")
