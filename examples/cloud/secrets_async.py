...
async def main() -> None:
    ...
    await delete_secret(cert.id)


async def upload_tls_cert() -> Secret:
...
async def get_secret_by_id(secret_id: str) -> Secret:
    gcore = AsyncGcore()
...
async def list_all_secrets() -> List[Secret]:
    gcore = AsyncGcore()
    all_secrets = await gcore.cloud.secrets.list()

    print("\n=== LIST ALL SECRETS ===")
    count = 1
    # TODO async for and remove .results after pagination
    for secret in all_secrets.results:
