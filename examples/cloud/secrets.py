...
def get_secret_by_id(secret_id: str) -> Secret:
    gcore = Gcore()
    secret = gcore.cloud.secrets.get(secret_id=secret_id)
...
def list_all_secrets() -> List[Secret]:
    gcore = Gcore()
    all_secrets = gcore.cloud.secrets.list()
...
def delete_secret(secret_id: str) -> None:
    gcore = Gcore()
    gcore.cloud.secrets.delete(secret_id=secret_id)
