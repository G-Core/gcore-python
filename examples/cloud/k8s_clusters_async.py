...
async def main() -> None:
    ...
    versions = await list_versions(client=gcore)
    cluster_version = _pick_create_version(versions)

    # Cluster lifecycle.
    cluster_name = await create_cluster(
        client=gcore,
    ...
    await get_cluster(client=gcore, cluster_name=cluster_name)
    await list_clusters(client=gcore)
    await update_cluster(client=gcore, cluster_name=cluster_name)

    # Upgrade the cluster to the newest available version, if one exists.
    upgrade_versions = await list_upgrade_versions(client=gcore, cluster_name=cluster_name)
    if upgrade_versions:
        await upgrade_cluster(client=gcore, cluster_name=cluster_name, target_version=upgrade_versions[-1].version)
        ...
        await wait_for_cluster_ready(client=gcore, cluster_name=cluster_name)
    else:
        print(f"No upgrade versions available for cluster {cluster_name}; skipping upgrade")

    await get_certificate(client=gcore, cluster_name=cluster_name)
    await get_kubeconfig(client=gcore, cluster_name=cluster_name)

    # Pool operations.
    await list_pools(client=gcore, cluster_name=cluster_name)
    first_pool = await _get_first_pool(client=gcore, cluster_name=cluster_name)

    await check_pool_quota(client=gcore, flavor_id=pool_flavor_id)
    second_pool = await create_pool(client=gcore, cluster_name=cluster_name, flavor_id=pool_flavor_id)
    await update_pool(client=gcore, cluster_name=cluster_name, pool_name=second_pool)
    await resize_pool(client=gcore, cluster_name=cluster_name, pool_name=second_pool)
    await list_pool_nodes(client=gcore, cluster_name=cluster_name, pool_name=second_pool)
    await delete_pool(client=gcore, cluster_name=cluster_name, pool_name=second_pool)

    # Ensure the initial pool is still reachable after the second pool is gone.
    await get_pool(client=gcore, cluster_name=cluster_name, pool_name=first_pool)

    await list_cluster_nodes(client=gcore, cluster_name=cluster_name)

    # Final cleanup.
    await delete_cluster(client=gcore, cluster_name=cluster_name)


async def create_cluster(*, client: AsyncGcore, flavor_id: str, version: str, keypair_name: str) -> str:
    print("\n=== CREATE K8S CLUSTER ===")
    cluster = await client.cloud.k8s.clusters.create_and_poll(
...
async def get_cluster(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== GET K8S CLUSTER ===")
    cluster = await client.cloud.k8s.clusters.get(cluster_name=cluster_name)
    print(
...
async def list_clusters(*, client: AsyncGcore) -> None:
    print("\n=== LIST K8S CLUSTERS ===")
    clusters = await client.cloud.k8s.clusters.list()
...
async def update_cluster(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== UPDATE K8S CLUSTER ===")
    cluster = await client.cloud.k8s.clusters.update_and_poll(
...
async def list_upgrade_versions(*, client: AsyncGcore, cluster_name: str) -> List[K8SClusterVersion]:
    print("\n=== LIST K8S CLUSTER UPGRADE VERSIONS ===")
    versions = await client.cloud.k8s.clusters.list_versions_for_upgrade(cluster_name=cluster_name)
...
async def upgrade_cluster(*, client: AsyncGcore, cluster_name: str, target_version: str) -> None:
    print("\n=== UPGRADE K8S CLUSTER ===")
    cluster = await client.cloud.k8s.clusters.upgrade_and_poll(cluster_name=cluster_name, version=target_version)
    print(f"Upgraded K8s cluster: name={cluster.name}, version={cluster.version}")
    print("===========================")


async def get_certificate(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== GET K8S CLUSTER CERTIFICATE ===")
    cert = await client.cloud.k8s.clusters.get_certificate(cluster_name=cluster_name)
    print(f"Certificate bytes={len(cert.certificate)}, Key bytes={len(cert.key)}")
    print("===================================")


async def wait_for_cluster_ready(*, client: AsyncGcore, cluster_name: str) -> None:
...
async def get_kubeconfig(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== GET K8S KUBECONFIG ===")
    kubeconfig = await client.cloud.k8s.clusters.kubeconfig.get(cluster_name=cluster_name)
    print(
...
async def list_pools(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== LIST K8S CLUSTER POOLS ===")
    pools = await client.cloud.k8s.clusters.pools.list(cluster_name=cluster_name)
...
async def get_pool(*, client: AsyncGcore, cluster_name: str, pool_name: str) -> None:
    print("\n=== GET K8S CLUSTER POOL ===")
    pool = await client.cloud.k8s.clusters.pools.get(pool_name=pool_name, cluster_name=cluster_name)
    print(
...
async def check_pool_quota(*, client: AsyncGcore, flavor_id: str) -> None:
    print("\n=== CHECK K8S POOL QUOTA ===")
    quota = await client.cloud.k8s.clusters.pools.check_quota(
        flavor_id=flavor_id,
...
async def create_pool(*, client: AsyncGcore, cluster_name: str, flavor_id: str) -> str:
    print("\n=== CREATE K8S CLUSTER POOL ===")
    pool = await client.cloud.k8s.clusters.pools.create_and_poll(
        cluster_name=cluster_name,
        name="gcore-python-pool-2",
        flavor_id=flavor_id,
        min_node_count=1,
...
async def update_pool(*, client: AsyncGcore, cluster_name: str, pool_name: str) -> None:
    print("\n=== UPDATE K8S CLUSTER POOL ===")
    pool = await client.cloud.k8s.clusters.pools.update(
        pool_name=pool_name,
        cluster_name=cluster_name,
...
async def resize_pool(*, client: AsyncGcore, cluster_name: str, pool_name: str) -> None:
    print("\n=== RESIZE K8S CLUSTER POOL ===")
    pool = await client.cloud.k8s.clusters.pools.resize_and_poll(
        pool_name=pool_name,
        cluster_name=cluster_name,
        node_count=2,
    )
...
async def delete_pool(*, client: AsyncGcore, cluster_name: str, pool_name: str) -> None:
    print("\n=== DELETE K8S CLUSTER POOL ===")
    await client.cloud.k8s.clusters.pools.delete_and_poll(pool_name=pool_name, cluster_name=cluster_name)
    print(f"Deleted pool: name={pool_name}")
    print("===============================")


async def list_cluster_nodes(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== LIST K8S CLUSTER NODES ===")
    nodes = await client.cloud.k8s.clusters.nodes.list(cluster_name=cluster_name)
...
async def list_pool_nodes(*, client: AsyncGcore, cluster_name: str, pool_name: str) -> None:
    print("\n=== LIST K8S POOL NODES ===")
    nodes = await client.cloud.k8s.clusters.pools.nodes.list(pool_name=pool_name, cluster_name=cluster_name)
    if not nodes.results:
...
async def delete_cluster(*, client: AsyncGcore, cluster_name: str) -> None:
    print("\n=== DELETE K8S CLUSTER ===")
    await client.cloud.k8s.clusters.delete_and_poll(cluster_name=cluster_name)
...
async def list_flavors(*, client: AsyncGcore) -> List[BaremetalFlavor]:
    print("\n=== LIST K8S FLAVORS ===")
    flavors = await client.cloud.k8s.flavors.list(exclude_gpu=True, include_capacity=True)
...
async def list_versions(*, client: AsyncGcore) -> List[K8SClusterVersion]:
    print("\n=== LIST K8S VERSIONS ===")
    versions = await client.cloud.k8s.list_versions()
    for count, version in enumerate(versions.results, 1):
...
async def _get_first_pool(*, client: AsyncGcore, cluster_name: str) -> str:
    pools = await client.cloud.k8s.clusters.pools.list(cluster_name=cluster_name)
    if not pools.results:
