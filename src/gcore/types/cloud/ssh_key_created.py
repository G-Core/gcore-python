...
class SSHKeyCreated(BaseModel):
    ...
    generate a key for you. The `private_key` will be returned \\**\\**once\\**\\** in the
    API response. Be sure to save it securely, as it cannot be retrieved again
    later. Best practice: Save the private key to a secure location on your machine
    (e.g., `~/.ssh/id_ed25519`) and set the file permissions to be readable only by
    you.
    """
